...
{	
				...
				if(BackMap[i][j] == 0 || BackMap[i][j] == 2 || BackMap[i][j] == 5)
				{
					...
					else if(Math.random() > 0.99)
					{
						...
						var actor = {
							...
							vampirism: Math.random() > 0.999 ? 1:0
						};
	...
	{
		if(!(actorList[actor].name == "Hugant" || actorList[actor].name == "Door" || actorList[actor].name == "Chest"))
		{
			activeActors.push(actorList[actor]);
		}
	}
}
...
function onTwink(actor)
{
	var xMap = player.x * 48;
	var yMap = player.y * 48;
	
	switch(BackMap[actor.y][actor.x])
	{
		case 1:
		context.drawImage(grass1I, xMap, yMap);
		break;
